...
def forecast_category_demand(purchase_orders: pd.DataFrame, periods: int = 3) -> pd.DataFrame:
    ...
    for category, frame in monthly.groupby("category"):
        ...
        if len(scoped) < 4:
            ...
            for step in range(1, periods + 1):
                next_month = scoped["month"].max() + pd.offsets.MonthBegin(step) if not scoped.empty else pd.Timestamp.today().floor("D")
                forecasts.append({"category": category, "month": next_month, "forecast_quantity": baseline, "model": "mean_fallback"})
            continue
        ...
        for step in range(1, periods + 1):
            ...
            row = pd.DataFrame({"month_idx": [last_idx + step], "month_num": [next_month.month], "quarter": [next_month.quarter]})
            pred = float(model.predict(row)[0])
            forecasts.append({"category": category, "month": next_month, "forecast_quantity": max(0.0, pred), "model": "random_forest"})

    return pd.DataFrame(forecasts)


def detect_procurement_anomalies(purchase_orders: pd.DataFrame, contamination: float = 0.03) -> pd.DataFrame:
    """Flag anomalous purchase orders using Isolation Forest on spend and operational features."""
    df = purchase_orders.copy()
    if df.empty:
        return df.assign(anomaly_score=pd.Series(dtype=float), is_anomaly=pd.Series(dtype=bool))

    model_input = pd.DataFrame(
        {
            "quantity": pd.to_numeric(df["quantity"], errors="coerce").fillna(0.0),
            "unit_price_ngn": pd.to_numeric(df["unit_price_ngn"], errors="coerce").fillna(0.0),
            "total_amount_ngn": pd.to_numeric(df["total_amount_ngn"], errors="coerce").fillna(0.0),
            "currency_is_usd": df.get("currency", "NGN").astype(str).eq("USD").astype(int),
        }
    )
