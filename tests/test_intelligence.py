...
class TestDemandForecastEngine:
    def test_forecast_returns_results(self):
        ...
        results = engine.forecast(
            df, date_col="order_date", value_col="unit_price", group_col="category"
        )
...
class TestInsightGenerator:
        ...
        context = {
            ...
            "analytics": {
                "category_spend": pd.DataFrame(
                    {"category": ["A", "B"], "total_spend": [3000, 2000]}
                ),
                "price_variance_top20": pd.DataFrame(
                    {"material": ["M1"], "variance_pct": [25.0]}
                ),
            },
            "forecasts": [],
        }
    ...
    def test_insight_fields(self):
        ...
        context = {
            "insights": {
                "total_spend_ngn": 1_000_000,
                "price_standardization_savings_ngn": 50_000,
                "on_time_delivery_pct": 90,
                "quality_incident_count": 5,
            },
            "analytics": {
                "category_spend": pd.DataFrame(),
                "price_variance_top20": pd.DataFrame(),
